...
### 4-4. 멀티캐스트(Multicast)
**멀티캐스트란?** 필요한 사람한테만 한 번에 보내는 방식
<details>
    <summary>특징</summary>

- 라우터가 트래픽을 필요한 곳으로만 전달
- 특정 멀티캐스트 그룹에 가입한 호스트만 수신
</details>
<details>
    <summary>장점</summary>
...
| **구분** | **유니캐스트** | **브로드캐스트** | **애니캐스트** | **멀티캐스트** |
| --- | --- | --- | --- | --- |
| **전송 대상** | 1명 | 같은 네트워크 전체 | 여러 중 1명 | 1:그룹 |
| **통신 방식** | 1:1 | 1:N | 1:1 | 그룹 선택 전송 |
| **효율성** | 높음 | 낮음 | 매우 높음 | 높음 |
| **속도** | 보통 | 느림 | 빠름 | 빠름 |
| **네트워크 범위** | 어디든 가능 | LAN만 가능 | 전세계 가능 | 그룹 참여자 |
| **대표 사용** | 웹 / API | ARP / DHCP | DNS / CDN | IPTV / 스트리밍 |
...
| 순서 보장 | O | X |
| 재전송 | O | X |
| 용도 | HTTP, HTTPS, FTP, SSH | DNS, DHCP, 유튜브 스트리밍 |
